...
public class TC01_CheckCreateBoard_POST extends Testbase{
    ...
    public void CheckCreatingBoardWithValidData_P() {
        ...
        response_create = (Response) given().log().all().queryParam("name", "board1").queryParam("key", key)
    ...
    public void CheckCreatingBoardWithInvalidHeader(){
        given().queryParam("name","board1").queryParam("key",key)
                .queryParam("token",token)
                .header("Content-Type","application/xml").when().post("/boards").then().statusCode(400);
    }
    ...
    public void CheckCreatingBoardWithInvalidMethod(){
        given().queryParam("name","board1").queryParam("key",key)
                .queryParam("token",token)
                .header("Host","").when().post("/boards").then().statusCode(405);
    ...
    public void CheckCreatingBoardWithAuth(){
        given().auth().basic("user","test test").queryParam("name","board1").queryParam("key",key)
                .queryParam("token",token)
                .header("Host","").when().post("/boards").then().statusCode(400); // or 405?
    }

    @Test(priority = 6, description = "Check name value")
    public void checkNameAsExpected_P(){

        Assert.assertEquals(response_create.jsonPath().getString("name"),"board1");
    }
